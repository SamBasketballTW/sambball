$(document).ready(function () {
    fetch('../data/standings-t1.csv')
        ...
        .then((result) => {
            ...
            rank = [];
            for (let i = 0; i < 5; i++) rank.push(t1_team[i + 1]);
            ...
            lines.forEach(player => {
                ...
                // calendar
                teams_info[teamW_index][18][gameDate.getMonth()][0] += 1;
                teams_info[teamL_index][18][gameDate.getMonth()][1] += 1;
                // lunar
                if (gameDate < lunarDate) {
                    teams_info[teamW_index][17][0][0] += 1
                    teams_info[teamL_index][17][0][1] += 1
                } else {
                    teams_info[teamW_index][17][1][0] += 1
                    teams_info[teamL_index][17][1][1] += 1
                }
                // streak
                ...
                if (q1_w + q2_w > q1_l + q2_l) {
                    teams_info[teamW_index][10][0] += 1;
                    teams_info[teamL_index][11][1] += 1;
                } else if (q1_w + q2_w < q1_l + q2_l) {
                    teams_info[teamW_index][11][0] += 1;
                    teams_info[teamL_index][10][1] += 1;
                } else {
                    teams_info[teamW_index][12][0] += 1;
                    teams_info[teamL_index][12][1] += 1;
                }

                if (q1_w + q2_w + q3_w > q1_l + q2_l + q3_l) {
                    teams_info[teamW_index][13][0] += 1;
                    teams_info[teamL_index][14][1] += 1;
                } else if (q1_w + q2_w + q3_w < q1_l + q2_l + q3_l) {
                    teams_info[teamW_index][14][0] += 1;
                    teams_info[teamL_index][13][1] += 1;
                } else {
                    teams_info[teamW_index][15][0] += 1;
                    teams_info[teamL_index][15][1] += 1;
                }

                if (pts_w - pts_l <= 3) {
                    teams_info[teamW_index][16][0][0] += 1;
                    teams_info[teamL_index][16][0][1] += 1;
                } else if (pts_w - pts_l >= 10) {
                    teams_info[teamW_index][16][2][0] += 1;
                    teams_info[teamL_index][16][2][1] += 1;
                } else {
                    teams_info[teamW_index][16][1][0] += 1;
                    teams_info[teamL_index][16][1][1] += 1;
                }
            ...
            for (let i = 0; i < rank.length; i++) matchup_thead += `<th style="width:70px">${short_teamName[teams_info[i][0]]}</td>`
            table.innerHTML += matchup_thead + `</thead>`;

            for (let i = 0; i < rank.length; i++) {
                if (teams_info[i][6][0] + teams_info[i][6][1] == 0) teams_info[i][6] = ['', ''];
                if (teams_info[i][12][0] + teams_info[i][12][1] == 0) teams_info[i][12] = ['', ''];
                if (teams_info[i][15][0] + teams_info[i][15][1] == 0) teams_info[i][15] = ['', ''];
                ...
                <tr class="filterTr t1">
                    ...
                    <td class="textL">
                        <img src="../asset/images/men/${teams_info[i][0]}.png" alt="${teams_info[i][0]}" class="teamicon">
                        <b>${short_teamName[teams_info[i][0]]}<a style="font-size:12px">${playoff[teams_info[i][0]]}</a></b>
                    </td>
                    ...
                    <td>${((teams_info[i][8][0] + teams_info[i][8][1]) / total_games).toFixed(1)}</td>
                    <td>${((teams_info[i][9][0] + teams_info[i][9][1]) / total_games).toFixed(1)}</td>
                </tr>`
                ...
                <tr class="filterTr t1">
                    ...
                    <td>${((teams_info[i][8][0] + teams_info[i][8][1]) / total_games).toFixed(1)}</td>
                    <td class="borderR">${((teams_info[i][9][0] + teams_info[i][9][1]) / total_games).toFixed(1)}</td>
                    <td>${(teams_info[i][7][0] / total_games).toFixed(1)}</td>
                    <td>${(teams_info[i][7][1] / total_games).toFixed(1)}</td>
                    <td>${(teams_info[i][7][2] / total_games).toFixed(1)}</td>
                    <td class="borderR">${(teams_info[i][7][3] / total_games).toFixed(1)}</td>
                    <td>${(teams_info[i][8][0] / teams_info[i][1][0]).toFixed(1)}</td>
                    <td class="borderR">${(teams_info[i][8][1] / teams_info[i][1][1]).toFixed(1)}</td>
                    <td>${(teams_info[i][9][0] / teams_info[i][1][0]).toFixed(1)}</td>
                    <td>${(teams_info[i][9][1] / teams_info[i][1][1]).toFixed(1)}</td>
                </tr>`
                ...
                <tr class="filterTr t1">
                    ...
                    <td>${teams_info[i][1][1]}</td>
                    <td class="borderR">${((teams_info[i][1][0] / total_games) * 100).toFixed(0)}%</td>
                    <td>${teams_info[i][10][0]}-${teams_info[i][10][1]}</td>
                    <td>${teams_info[i][11][0]}-${teams_info[i][11][1]}</td>
                    <td class="borderR">${teams_info[i][12][0]}-${teams_info[i][12][1]}</td>
                    <td>${teams_info[i][13][0]}-${teams_info[i][13][1]}</td>
                    <td>${teams_info[i][14][0]}-${teams_info[i][14][1]}</td>
                    <td class="borderR">${teams_info[i][15][0]}-${teams_info[i][15][1]}</td>
                    <td>${teams_info[i][16][0][0]}-${teams_info[i][16][0][1]}</td>
                    <td>${teams_info[i][16][1][0]}-${teams_info[i][16][1][1]}</td>
                    <td>${teams_info[i][16][2][0]}-${teams_info[i][16][2][1]}</td>
                </tr>`
                ...
                for (let j = 0; j < rank.length; j++) {
                    ...
                    match_standings += `<td>${teams_info[i][19][j][1]}-${teams_info[i][19][j][2]}</td>`
                }
                table.innerHTML += `
                <tbody>
                    <tr>
                ...
                <tr>
                    ...
                    <td>${teams_info[i][1][1]}</td>
                    <td class="borderR">${((teams_info[i][1][0] / total_games) * 100).toFixed(0)}%</td>
                    <td>${teams_info[i][17][0][0]}-${teams_info[i][17][0][1]}</td>
                    <td class="borderR">${teams_info[i][17][1][0]}-${teams_info[i][17][1][1]}</td>
                    <td>${teams_info[i][18][9][0]}-${teams_info[i][18][9][1]}</td>
                    <td>${teams_info[i][18][10][0]}-${teams_info[i][18][10][1]}</td>
                    <td>${teams_info[i][18][11][0]}-${teams_info[i][18][11][1]}</td>
                    <td>${teams_info[i][18][0][0]}-${teams_info[i][18][0][1]}</td>
                    <td>${teams_info[i][18][1][0]}-${teams_info[i][18][1][1]}</td>
                    <td>${teams_info[i][18][2][0]}-${teams_info[i][18][2][1]}</td>
                    <td>${teams_info[i][18][3][0]}-${teams_info[i][18][3][1]}</td>
                </tr>`
            }
        });
});